...
def calculate_pv_after_commission(w1, w0, commission_rate):
    ...
    """
    mu0 = 1
    mu1 = commission_rate*(np.sum(abs(w0[1:] - mu0*w1[1:]))) #Candice: Changed based on the paper
    Cs=commission_rate-0.001 #Candice: Changed Cs and Cp
    Cp=commission_rate+0.001

    while abs(mu1-mu0) > 1e-10:
        mu0 = mu1
        mu1 = (1 - Cp * w0[0] -
            (2 * commission_rate - Cs*Cp) *
